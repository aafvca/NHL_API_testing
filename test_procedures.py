...
def roster_data(url_team):
    roster_data, status_code = tf.get_response(url_team)
    return roster_data, status_code
...
def roster_points(url_people,players_in_both,url_stats,api_points):
    roster_points, empty_value, status_code = tf.create_list_multiple(url_people,players_in_both,url_stats,api_points)
    return roster_points, empty_value, status_code
...
def people_current_team(url_people,roster_1718,key,api_people_currentTeam):
    people_current_team, t3_empty_value_current, status_code  = tf.create_list_multiple(url_people,roster_1718,key,api_people_currentTeam)
    return people_current_team, t3_empty_value_current, status_code
...
def people_position(url_people,roster_1718,key,api_people_position):
    people_position, t3_empty_value_position, status_code = tf.create_list_multiple(url_people,roster_1718,key,api_people_position)
    return people_position, t3_empty_value_position, status_code
